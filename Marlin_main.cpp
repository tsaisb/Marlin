...
const float default_bed_level[7][7] = {
  { -0.375, -0.425, -0.187, -0.275, 0.000, -0.087, 0.225,  },
  { -0.375, -0.425, -0.312, -0.450, -0.125, -0.087, 0.225,  },
  { -0.375, -0.450, -0.425, -0.462, -0.175, -0.162, 0.225,  },
  { -0.287, -0.387, -0.387, -0.362, -0.250, -0.012, 0.188,  },
  { -0.112, -0.262, -0.375, -0.062, -0.225, 0.088, 0.400,  },
  { -0.112, 0.038, -0.225, 0.013, -0.112, 0.238, 0.400,  },
  { -0.112, 0.038, 0.150, 0.275, 0.238, 0.238, 0.400,  }
};
...
void mean_calibrate_print_surface(float z_offset) {
  ...
  */
  // Print calibration results for manual frame adjustment.
  SERIAL_PROTOCOLPGM("after mean calibration");
  SERIAL_ECHOLN("");
  for(int y = 3; y >= -3; y--) {
      for (int x = -3; x <= 3; x++) {
        SERIAL_PROTOCOL_F(bed_level[x+3][y+3], 3);
...
void process_commands()
{
  ...
  if(code_seen('G'))
  {
    ...
    case 30: // G30 mean bed level calibrate
        saved_feedrate = feedrate*3.5;
        saved_feedmultiply = feedmultiply;
        feedmultiply = 100;
        deploy_z_probe();
        mean_calibrate_print_surface(z_probe_offset[Z_AXIS] + (code_seen(axis_codes[Z_AXIS]) ? code_value() : 0.0));
        retract_z_probe();
